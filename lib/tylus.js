...
var T = (function () {
  ...
  var screenInfo = function () {  
    ...
    return {
      ...
      diag: Math.sqrt(
        Math.pow((screenWidth / dpi), 2) + Math.pow((screenWidth / dpi), 2)
      ),
      dpi: dpi,
  ...
  try {
    ...
    if (info.diag > 7) properties.formfactor = 'tablet';
    else if (info.diag > 2.7) properties.formfactor = 'phone';
    ...
    if (_) {
      ...
      for (var prop in computedStyle) if (computedStyle.hasOwnProperty(prop)) {
        ...
        if (typeof(computedStyle[prop]) == "function") {
          newStyle[prop] = computedStyle[prop](strings);
        } else {
